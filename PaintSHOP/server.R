...
shinyServer(function(input, output) {
    ...
    coord_intersect <- reactive({
      #browser()
      # I need to change to take either manual or file  
      result <- genome_intersect(probes_full(), coordinates(), by = c("chrom", "start", "stop"))
        
        # reorder columns after intersect
        result %>%
          select(chrom, start, stop, everything())
    })
